...
#include "application_manager/resumption/resumption_process.h"
...
ResumptionProcess::ResumptionProcess(
...
void ResumptionProcess::Restore(ApplicationSharedPtr application,
...
void ResumptionProcess::on_event(const event_engine::Event& event) {
  LOG4CXX_AUTO_TRACE(logger_);
  const smart_objects::SmartObject& message = event.smart_object();

  const int32_t app_id = message[strings::msg_params][strings::app_id].asUInt();

  ResumptionStatus& status = resumption_status_[app_id];
  std::vector<ResumptionRequest>& list_of_sent_requests =
      status.list_of_sent_requests;

  ResumptionRequest response;
  response.correlation_id = event.smart_object_correlation_id();
  response.function_id = event.id();
  response.message = message;

  auto request_ptr =
      std::find_if(list_of_sent_requests.begin(),
                   list_of_sent_requests.end(),
                   [&response](const ResumptionRequest& request) {
                     return request.correlation_id == response.correlation_id &&
                            request.function_id == response.function_id;
                   });
  ...
  if (hmi_apis::Common_Result::SUCCESS == result_code) {
    ...
    status.erro_requests.push_back(*request_ptr);
  }
  list_of_sent_requests.erase(request_ptr);

  if (list_of_sent_requests.empty()) {
    if (!status.erro_requests.empty()) {
      LOG4CXX_ERROR(logger_, "Resumption for app " << app_id << "failed");
      RevertRestoredData(app_id);
    } else {
      LOG4CXX_ERROR(logger_, "Resumption for app " << app_id << "successful");
    }
    resumption_status_.erase(app_id);
  }
}

void ResumptionProcess::RevertRestoredData(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
  DeleteFiles(app_id);
...
void ResumptionProcess::WaitForResponse(const int32_t app_id,
...
bool ResumptionProcess::ProcessHMIRequest(
    smart_objects::SmartObjectSPtr request, bool use_events) {
  LOG4CXX_AUTO_TRACE(logger_);
  if (use_events) {
...
void ResumptionProcess::ProcessHMIRequests(
    const smart_objects::SmartObjectList& requests) {
  for (const auto& it : requests) {
    ProcessHMIRequest(it, /*use_events = */ true);
  }
}

void ResumptionProcess::AddFiles(ApplicationSharedPtr application,
...
void ResumptionProcess::DeleteFiles(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
...
void ResumptionProcess::AddSubmenues(
...
void ResumptionProcess::DeleteSubmenues(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
  ResumptionStatus& status = resumption_status_[app_id];
  ApplicationSharedPtr application = application_manager_.application(app_id);
  for (auto request : status.successful_requests) {
    if (hmi_apis::FunctionID::UI_AddSubMenu == request.function_id) {
      ...
      ProcessHMIRequest(ui_sub_menu, /*use_events = */ false);
    }
  }
}

void ResumptionProcess::AddCommands(
    ApplicationSharedPtr application,
    const smart_objects::SmartObject& saved_app) {
  LOG4CXX_AUTO_TRACE(logger_);
...
void ResumptionProcess::DeleteCommands(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
  ApplicationSharedPtr application = application_manager_.application(app_id);
  ResumptionStatus& status = resumption_status_[app_id];
  for (auto request : status.successful_requests) {
...
void ResumptionProcess::DeleteUICommands(const ResumptionRequest& request) {
  LOG4CXX_AUTO_TRACE(logger_);
  ...
  ProcessHMIRequest(ui_command, /*use_events = */ false);
}

void ResumptionProcess::DeleteVRCommands(const ResumptionRequest& request) {
  LOG4CXX_AUTO_TRACE(logger_);
  ...
  ProcessHMIRequest(vr_command, /*use_events = */ false);
}

void ResumptionProcess::AddChoicesets(
    ApplicationSharedPtr application,
    const smart_objects::SmartObject& saved_app) {
  LOG4CXX_AUTO_TRACE(logger_);
...
void ResumptionProcess::DeleteChoicesets(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
  ApplicationSharedPtr application = application_manager_.application(app_id);
...
void ResumptionProcess::SetGlobalProperties(
    ApplicationSharedPtr application,
    const smart_objects::SmartObject& saved_app) {
  LOG4CXX_AUTO_TRACE(logger_);
...
void ResumptionProcess::DeleteGlobalProperties(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
}

void ResumptionProcess::AddWayPointsSubscription(
...
void ResumptionProcess::DeleteWayPointsSubscription(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
...
void ResumptionProcess::AddSubscriptions(
    ApplicationSharedPtr application,
    const smart_objects::SmartObject& saved_app) {
  LOG4CXX_AUTO_TRACE(logger_);
...
void ResumptionProcess::DeleteSubscriptions(const int32_t app_id) {
  LOG4CXX_AUTO_TRACE(logger_);
  ApplicationSharedPtr application = application_manager_.application(app_id);
