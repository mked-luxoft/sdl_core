...
bool RPCProtectionMediatorImpl::CheckPolicyEncryptionFlag(
  ...
  for (const auto& group : app_rpc_groups) {
    ...
    if (is_function_in_group &&
        rpc_encryption_manager.GroupNeedEncryption(group)) {
      LOG4CXX_DEBUG(logger_,
...
// bool RPCProtectionMediatorImpl::CheckNegativeResponse(
//     const uint32_t app_id, const uint32_t correlation_id) {
//   LOG4CXX_AUTO_TRACE(logger_);

//   auto it = negative_responses_.find(std::make_pair(app_id, correlation_id));
//   if (it != negative_responses_.end()) {
//     negative_responses_.erase(it);
//     return true;
//   }
//   return false;
// }

void RPCProtectionMediatorImpl::EncryptResponseByForce(
    const uint32_t app_id, const uint32_t correlation_id) {
  message_needed_encryption_.insert(std::make_pair(app_id, correlation_id));
};

smart_objects::SmartObjectSPtr
RPCProtectionMediatorImpl::CreateEncryptionNeededResponse(
